...
import { Camera, CameraOff, ChevronRight, CircleHelp, Maximize2, Pause, Play, RotateCcw, Settings, Volume2, VolumeX, X, Zap } from 'lucide-react';

type GameMode = 'ready' | 'playing' | 'paused' | 'over';
type Point = { x: number; y: number; t: number };
type Fruit = { id: number; kind: string; x: number; y: number; vx: number; vy: number; radius: number; rotation: number; spin: number; sliced: boolean; bomb: boolean };
...
export default function FruitGame() {
  ...
  const startGame = useCallback(() => {
    ...
    lastSpawnRef.current = performance.now(); gameStartedRef.current = performance.now();
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      hands.onResults((results: any) => {
        ...
        (results.multiHandLandmarks || []).slice(0, 2).forEach((landmarks: any[], index: number) => {
          const palm = landmarks[9] || landmarks[0];
          trailsRef.current[index].push({ x: 1 - palm.x, y: palm.y, t: now });
          trailsRef.current[index] = trailsRef.current[index].filter((point) => now - point.t < 230).slice(-12);
        });
      });
  ...
  useEffect(() => {
    ...
    const spawn = (width: number, height: number, now: number) => {
      const count = Math.random() > 0.76 ? 2 : 1;
      for (let i = 0; i < count; i++) { const bomb = scoreRef.current > 50 && Math.random() < 0.09; fruitRef.current.push({ id: idRef.current++, kind: bomb ? '💣' : FRUITS[Math.floor(Math.random() * FRUITS.length)], x: width * (0.18 + Math.random() * 0.64), y: height + 50, vx: width * (-0.08 + Math.random() * 0.16), vy: -height * (0.72 + Math.random() * 0.22), radius: Math.max(25, width * 0.032), rotation: Math.random() * 4, spin: -2 + Math.random() * 4, sliced: false, bomb }); }
    ...
    const draw = (time: number) => {
      ...
      if (modeRef.current === 'playing') {
        ...
        fruitRef.current.forEach((fruit) => { fruit.vy += height * 1.12 * dt; fruit.x += fruit.vx * dt; fruit.y += fruit.vy * dt; fruit.rotation += fruit.spin * dt; if (!fruit.sliced && fruit.y > height + fruit.radius * 2 && fruit.vy > 0) { fruit.sliced = true; if (!fruit.bomb) { livesRef.current -= 1; setLives(livesRef.current); comboRef.current = 0; setCombo(0); ping(110, 0.15); } } });
      ...
      fruitRef.current = fruitRef.current.filter((fruit) => !fruit.sliced && fruit.y < height + 140);
      fruitRef.current.forEach((fruit) => { ctx.save(); ctx.translate(fruit.x, fruit.y); ctx.rotate(fruit.rotation); ctx.shadowColor = fruit.bomb ? '#ff4d4d' : 'rgba(0,0,0,.5)'; ctx.shadowBlur = fruit.bomb ? 24 : 12; ctx.font = `${fruit.radius * 1.65}px system-ui, Apple Color Emoji`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(fruit.kind, 0, 0); ctx.restore(); });
  ...
  return <main className="game-shell">
    ...
    <header className="topbar">
      ...
      <div className="header-actions"><button className="icon-button" onClick={() => setHelpOpen(true)} aria-label="Как играть"><CircleHelp size={20} /></button><button className="icon-button" onClick={() => setSettingsOpen(true)} aria-label="Настройки"><Settings size={20} /></button></div>
    </header>
    ...
    {(settingsOpen || helpOpen) && <div className="modal-backdrop" role="presentation" onMouseDown={() => { setSettingsOpen(false); setHelpOpen(false); }}><section className="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title" onMouseDown={(e) => e.stopPropagation()}><button className="modal-close" onClick={() => { setSettingsOpen(false); setHelpOpen(false); }} aria-label="Закрыть"><X size={19} /></button>
      {helpOpen ? <><span className="modal-kicker">КАК ИГРАТЬ</span><h2 id="modal-title">Твоя рука — лезвие</h2><ol className="how-list"><li><b>1</b><span>Разреши доступ к камере или просто двигай мышью по полю.</span></li><li><b>2</b><span>Делай быстрые взмахи через фрукты. Медленные движения не считаются.</span></li><li><b>3</b><span>Не пропускай фрукты и обходи бомбы. Серии дают больше очков.</span></li></ol></> : <><span className="modal-kicker">НАСТРОЙКИ</span><h2 id="modal-title">Подстрой игру</h2><label className="setting-row"><span><b>Камера</b><small>Управление двумя руками</small></span><input type="checkbox" checked={cameraOn} onChange={(e) => e.target.checked ? startCamera() : stopCamera()} /></label><label className="setting-row"><span><b>Звук</b><small>Сигналы разреза и промаха</small></span><input type="checkbox" checked={soundOn} onChange={(e) => setSoundOn(e.target.checked)} /></label><label className="setting-range"><span><b>Сложность</b><small>{difficulty === 1 ? 'Спокойно' : difficulty < 1.6 ? 'Быстро' : 'Турбо'}</small></span><input type="range" min="1" max="2" step="0.5" value={difficulty} onChange={(e) => setDifficulty(Number(e.target.value))} /></label><label className="setting-row"><span><b>Режим разработчика</b><small>FPS, объекты и отслеживание</small></span><input type="checkbox" checked={debug} onChange={(e) => setDebug(e.target.checked)} /></label></>}
    </section></div>}
  </main>;
}
